...
- **Verrou Pass 2** : `metadata_json.pass2_unlocked` (bool). Tant que `false`/absent → fiche « note augmentée » (intercalaire, titre, moment, mémo) + CTA i18n par type ; clic → `patchMetadata` + fondu vers UI complète (liste, jalons, itinéraire, Mission/Newton TRIP, etc.). Path B capture : même pose de `pass2_unlocked` avant enrich LIST/PROJECT.
