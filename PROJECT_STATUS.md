...
- **Cluster tactique (Tirelire)** : [`getBestOrphanCluster`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/services/clusterEngine.ts) sur `TimelineScreen` (seuil d’affichage `count >= 2`, contexte ALL + TODO) ; carte neumorphique + ouverture [`IdeaBankModal`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/IdeaBankModal.tsx) filtrée par `category_id` ; log `[CLUSTER-ENGINE]`.
- Alignement SPEC : le flux “**Micro as Bulk(1)**” est **unifié** : micro/texte unitaire passent par le **séquenceur bulk** avec persistance **ventilée** (une seule “source de vérité”), et un `traceId` est propagé pour des logs cohérents ; sur **TalkDebug**, l’**overlay de progression** ([`useAIProgressInertia`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/hooks/useAIProgressInertia.ts) + [`AIUniversalProgressOverlay`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/AIUniversalProgressOverlay.tsx) + événements `CAPTURE_PIPELINE_PROGRESS`) couvre l’attente Pass 1 (micro « échap » sans annuler le pipeline). **Correction STT optionnelle** : crayon sur [`TalkCaptureMicButton`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/TalkCaptureMicButton.tsx) avant Envoyer → `transcript` final vers Gemini ; logs `transcript_manual_edit` / `[MIC] ✏️`.
...
- `TalkDebugScreen.tsx` : écran principal de capture (debug-friendly) :
  ...
  - **Micro — correction STT (optionnelle)** : pendant la dictée, icône **Crayon** entre Pause et Envoyer → `CaptureTranscriptEditor` (lecture / `TextInput` multiline) ; STT gelé en édition ; **Envoyer** envoie le texte corrigé. Même barre sur **Timeline** (`TalkCaptureMicButton` variante compacte). Logs `[CAPTURE_FLOW]` `transcript_edit_start` / `transcript_manual_edit` + `[MIC] ✏️` (`transcript_original` vs `transcript_final`).
...
- **Verrou Pass 2** : `metadata_json.pass2_unlocked` (bool). Tant que `false`/absent → fiche **Zen** (intercalaire, titre, moment, mémo) + CTA i18n par type (`intentionDetail.pass2*`, ex. *Générer le plan* / *Générer la liste* / *Ajouter une note* pour PROJECT / LIST / TASK). **PRO** : clic → `patchMetadata` + fondu vers UI complète (liste, jalons, itinéraire, Mission/Newton TRIP, etc.) ; Path B capture : même pose de `pass2_unlocked` avant enrich LIST/PROJECT. **Talk — trajet en full sans Pass 2** : en `validationMode` + `peekCapturePhase === 'path_b'` + intention trajet + `sheetPosition === 'full'`, **`gateFullTripBypass`** évite que `gateLocked` conserve la branche simplifiée ; `useLayoutEffect` force `pass2RevealAnim` → **1** lorsque `pass2Unlocked || gateFullTripBypass` pour afficher itinéraire / transport / bloc Newton (Newton **`disabled`** tant que non surveillable — visible mais inactif si données incomplètes).
- **Path B — bouton principal contextuel (Talk)** : libellés / actions selon type (`talkDebug.actionSetupTrip`, `actionGeneratePlan`, `actionGenerateList`, `actionAddNote`, etc.) ; log dev **`[ACTION-ADVISOR]`** pour audit des choix CTA.
...
- **Overlay progression Talk** : [`useAIProgressInertia.ts`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/hooks/useAIProgressInertia.ts) + [`AIUniversalProgressOverlay.tsx`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/AIUniversalProgressOverlay.tsx) (exporte aussi **`CaptureTranscriptEditor`** pour le micro), branché depuis [`TalkDebugScreen.tsx`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/screens/TalkDebugScreen.tsx) ; phase micro **`pipeline_wait`** + ref **`exitPipelineWaitToIdle`** dans [`TalkCaptureMicButton.tsx`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/TalkCaptureMicButton.tsx). L’édition STT a lieu **avant** cet overlay (pendant `recording`). Le composant léger [`TalkPipelineProgressDashboard.tsx`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/TalkPipelineProgressDashboard.tsx) reste dans le dépôt (charte carte / barre) mais **n’est plus** l’overlay monté par TalkDebug.
