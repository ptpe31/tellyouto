...
- **TRIP — Sentinel Focus Badge (juin 2026)** : badge unique flottant (ombre 3D) pour le trajet actif ou la suggestion « Me prévenir quand partir » — [`SentinelFocusBadge`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/SentinelFocusBadge.tsx), [`useSentinelFocus`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/hooks/useSentinelFocus.ts), [`sentinelFocusSelection`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/utils/sentinelFocusSelection.ts) ; intégré sous « Aujourd’hui » (hub `EMAIL_HUB`) et sur **TalkDebug** au-dessus du micro ; i18n `sentinelFocus.prompt`. Détail : **SPEC.md § 8.c**.
...
- **Corps Timeline** (mode EMAIL_HUB) : `groupBy category_id` natif Pass 1 — blocs non vides uniquement ; lignes email avec heure + 🔁 habitude (JIT).
- **Sentinel Focus Badge** (juin 2026) : sous le titre « Aujourd’hui », avant les blocs catégorie — état A (capsule + GPS + réveil) ou B (incitation configuration) ; même composant sur **TalkDebug** (`todayFocusRows` + `INTENTIONS_CHANGED`).
