...
- **TRIP — Alarme départ capsule (mai 2026)** : slot réveil sur [`ElasticDepartureCapsule`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/ElasticDepartureCapsule.tsx) si promesse P1 (`showAlarmIcon`) → [`AlarmService.openAlarmSelection`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/services/alarmService.ts) ; heure prédéfinie **20 % avant `endMs`** (`resolveElasticDepartureAlarmUnixSec`, Timeline + sheet + badge) ; i18n `tripAlarm.*` ; queries [`app.json`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/app.json).
...
- **Sentinel Focus Badge** (juin 2026) : sous « Aujourd’hui », avant les blocs — bulle Talk 105 dp ; scan actif (capsule + réveil) ou suggestion cascade (`promptLine1` / `promptLine2`) ; micro expiré libère le slot pour le trajet suivant ; **TalkDebug** (`openTalkTripDetail` → sheet principale).
- **Perf** : `getItemLayout` off en mode hub ; hauteur item `sentinelFocus` = **117 dp** ou **0** (`SENTINEL_FOCUS_SLOT_HEIGHT` / `estimateSentinelFocusBadgeHeight`).
