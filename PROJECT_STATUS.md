...
- **Smart Clusters (Timeline, mai 2026)** : carrousel **Inbox · À acheter · Box · Routines · Projets** ([`SmartClustersCarousel`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/SmartClustersCarousel.tsx)) — **Box** = stock TODO sans date (hors Inbox jour, SHOP, HABIT) + purge globale ; **Routines** = bibliothèque HABIT par catégorie + badges série 🔥/❄️, sans purge globale ; hub **Aujourd’hui** (EMAIL_HUB) = exécution JIT des habitudes du jour. **Legacy** : [`getBestOrphanCluster`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/services/clusterEngine.ts) non branché. **Projets orphelins** (IdeaBank) : `cluster.planProjectStart` → `project.start_date` + replan jalons.
...
- **TRIP — Optimisation API Maps (mai 2026)** : saisie adresse **Lazy-Fetch** — **0 Autocomplete à la frappe** ; Geocoding au **Done** (< 12 car.) ; loupe volontaire (≥ 12 car.) → liste à plat → Place Details ; encoche verte + verrouillage post-validation ([`useAddressLogic`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/hooks/useAddressLogic.ts), [`AddressInput`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/traffic/AddressInput.tsx), [`addressResolver.ts`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/services/addressResolver.ts)) ; cache Distance Matrix **grid 3 décimales (~100 m)** + AsyncStorage TTL 15 min ; plafond PROBE1 **`probe1_retry_count ≥ 3` → STATIC** ; `patchMetadata` réentrant. Détail : **SPEC.md § 8.a**.
...
- **TRIP — Alarme départ capsule (mai 2026)** : slot réveil sur [`ElasticDepartureCapsule`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/ElasticDepartureCapsule.tsx) si promesse P1 (`showAlarmIcon`) → [`AlarmService.openAlarmSelection`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/services/alarmService.ts) (intent Android `SET_ALARM`, Horloge iOS) ; i18n `tripAlarm.*` ; queries [`app.json`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/app.json).
- **TRIP — Sentinel Focus Badge (juin 2026)** : micro-dashboard éphémère ([`SentinelMicroDashboard`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/SentinelMicroDashboard.tsx)) + suggestion « Me prévenir… » ([`SentinelFocusBadge`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/SentinelFocusBadge.tsx)) sous « Aujourd’hui » et **TalkDebug** ; filtre fraîcheur `nowMs < arrivalDue` ; `onPressSuggestion` → `IntentionDetailSheet` (même CTA setup TRIP). Détail : **SPEC.md § 8.c**.
- **Living Hub — bloc TRAJETS unifié (juin 2026)** : court-circuit UI [`resolveHubBlockCategoryId`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/features/livingHub/hubCategoryRegistry.ts) — tous les TRIP → `TRIPS_HUB` (🏁), pas le `category_id` Pass 1 (LEARN/SHOP/…). Détail : **SPEC.md § 2.c**.
...
| [`DebugScreen.tsx`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/screens/DebugScreen.tsx) | Showroom **🎨 EXPLORATION GRAPHIQUE** — 6 variantes + **Disposition Timeline** (`CURRENT` / `EMAIL_HUB`) |
...
| [`alarmService.ts`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/services/alarmService.ts) | Intent natif réveil départ (`openAlarmSelection`) — Android/iOS |
...
| [`ElasticDepartureCapsule.tsx`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/ElasticDepartureCapsule.tsx) | Capsule contrat : piste D, deadline, GPS (`onNavigationPress`), slot réveil (`showAlarmIcon` / `onAlarmPress`), état retard |
| [`SentinelFocusBadge.tsx`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/SentinelFocusBadge.tsx) | Badge unique trajet du jour (hub + Talk) ; ombre 3D ; états actif / suggestion |
| [`useSentinelFocus.ts`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/hooks/useSentinelFocus.ts) | Hook React — priorité scan actif puis suggestion |
| [`sentinelFocusSelection.ts`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/utils/sentinelFocusSelection.ts) | Sélecteur pur `pickSentinelFocus` / éligibilité trajet |
...
#### Living Hub — disposition email (Debug, mai 2026)

- **Switch Debug** : `CURRENT` (cartes) vs `EMAIL_HUB` — AsyncStorage `@trankil_debug_timeline_layout` ; module [`src/features/livingHub/`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/features/livingHub/) débranchable.
- **Carrousel** (vue Aujourd’hui + ALL + TODO) : **Inbox · À acheter · Box · Routines · Projets** — voir sections Box / Routines ci-dessous.
- **Corps Timeline** (mode EMAIL_HUB) : `groupBy` via `resolveHubBlockCategoryId` — blocs non vides ; **TRIP → `TRIPS_HUB`** (🏁 TRAJETS) en premier, puis catégories Pass 1 ; lignes email + 🔁 habitude (JIT).
- **Sentinel Focus Badge** (juin 2026) : sous « Aujourd’hui », avant les blocs — micro-dashboard éphémère (barre compacte) ou suggestion (CTA setup) ; **TalkDebug** (`openTalkTripDetail` → sheet principale).
- **Perf** : `getItemLayout` off en mode hub ; hauteur item `sentinelFocus` estimée quand le badge est visible.

#### Box — stock sans date (mai 2026)

- **Carrousel** : tuile **Box** (hardcodée) avec pastille `boxCount` — remplace le nudge cluster orphelin + tuile Listes ; **exclut les HABIT**.
...
- **API** : `listTrankilV2BoxStockIntentions`, `bulkDeleteTrankilV2IntentionsByIds`, filtre `BOX_STOCK_WHERE` (TODO sans date, hors Inbox jour, hors SHOP, hors HABIT).
...
### 3.1b Timeline — Smart Clusters (Box · Routines · Inbox)
...
- **PRO + PROBE1 fait + mission surveillée active** : [`ElasticDepartureCapsule`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/ElasticDepartureCapsule.tsx) compacte remplace le badge texte ; GPS + réveil (si promesse P1) ; mapping via [`tripElasticCapsuleModel.ts`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/utils/tripElasticCapsuleModel.ts) ; tap GPS = navigation ; tap réveil = [`AlarmService`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/services/alarmService.ts).
