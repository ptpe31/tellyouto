...
- **Verrou Pass 2** : `metadata_json.pass2_unlocked` (bool). Tant que `false`/absent → fiche **Zen** (intercalaire, titre, moment, mémo). **CTA footer** (`pass2FooterCtaNode`, `footerActionsRow` / `footerRow`) : affiché seulement pour **TRIP / LIST / PROJECT** (`pass2FooterAction`) — libellés `intentionDetail.actionSetupTrip`, `pass2List`, `pass2ProjectSteps` ; **TASK** et **HABIT** : pas de CTA Pass 2 (note / habitude simple). **PRO** : fondu bouton (~220 ms) → `patchMetadata` + fondu corps (~320 ms) → enrichissement Gemini **LIST/PROJECT** si applicable ; **FREE** : suffixe `pass2LockedSuffix` (🔒), redirection `ProSubscription`. Path B capture Talk : bouton principal contextuel inchangé (`talkDebug.action*`). **Talk — trajet en full sans Pass 2** : `gateFullTripBypass` + `pass2RevealAnim` comme avant (itinéraire / Newton visibles ; CTA footer trajet si `pass2_unlocked` encore faux).
