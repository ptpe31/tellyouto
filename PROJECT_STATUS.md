...
- **`IntentionDetailSheet.tsx`** : footer CTA TRIP/LIST/PROJECT ; `pass2_unlocked: 1` au clic PRO ; overlay [`useAIProgressInertia`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/hooks/useAIProgressInertia.ts) + [`AIUniversalProgressOverlay`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/AIUniversalProgressOverlay.tsx) pour enrichissement LIST/PROJECT (i18n `pass2.*`) ; `applyPass2MetadataLocally` + `onPatchRow` pour affichage instantané items/jalons ; `onPressPass2` (micro Path B) partage le même flux ; payload court + SI LIST/PROJECT dans [`geminiSemanticLab.ts`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/services/geminiSemanticLab.ts) ; logs dev `[Pass2] ✅ … enrich …ms`.
...
- **Verrou Pass 2** : `metadata_json.pass2_unlocked` (**compteur binaire** : absent/`0` = Zen, **`1`** = génération consommée). **CTA footer** masqué si `pass2_unlocked === 1` ; visible pour **TRIP | LIST | PROJECT** uniquement (`showPass2FooterCta`). Libellés `intentionDetail.actionSetupTrip`, `pass2List`, `pass2ProjectSteps`. **TASK/HABIT** : pas de CTA. **PRO** : `pass2_unlocked: 1` au clic → overlay inertie (LIST/PROJECT) → hydratation locale items/jalons → fondu révélation ; réouverture avec `=== 1` → vue détaillée immédiate. **FREE** : 🔒 + `ProSubscription`. Path B Talk : `onPressPass2` / `talkDebug.action*`. **Trajet full bypass** : `gateFullTripBypass` ; CTA tant que `!== 1`.
