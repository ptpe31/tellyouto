...
- **Verrou Pass 2** : `metadata_json.pass2_unlocked` (bool). Tant que `false`/absent → fiche **Zen** (intercalaire, titre, moment, mémo) + CTA i18n par type. **PRO** : clic → `patchMetadata` + fondu vers UI complète (liste, jalons, itinéraire, Mission/Newton TRIP, etc.) ; Path B capture : même pose de `pass2_unlocked` avant enrich LIST/PROJECT. **Talk — trajet en full sans Pass 2** : en `validationMode` + `peekCapturePhase === 'path_b'` + intention trajet + `sheetPosition === 'full'`, **`gateFullTripBypass`** évite que `gateLocked` conserve la branche simplifiée ; `useLayoutEffect` force `pass2RevealAnim` → **1** lorsque `pass2Unlocked || gateFullTripBypass` pour afficher itinéraire / transport / bloc Newton (Newton **`disabled`** tant que non surveillable — visible mais inactif si données incomplètes).
