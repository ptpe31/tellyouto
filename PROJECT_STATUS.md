...
| **UI Inbox** | [`InboxLineTitle.tsx`](src/components/InboxLineTitle.tsx) — 2 lignes strictes, pastille catégorie ; L2 zoom interactive (étapes + chevron) ; **pas de miniature Vault** (perf FlatList) |
| **Hiérarchie Inbox** | [`buildInboxRootsView`](src/utils/inboxRootsView.ts) — filtrage racines **côté JS** ; accordéon sourcing + voyage + **zoom décomposé** (`zoomInboxModel.ts`, Option A, chevron L2) dans `IdeaBankModal` |
