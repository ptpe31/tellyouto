...
- **Inbox** : [`INBOX_TODAY_WHERE`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/api/trankilV2Db.ts) — journal 24h (`created_at` = jour local, `status = TODO`) + merge `listTrankilV2InboxZoomChildTasksForDay` (TODO/DONE) ; [`IdeaBankModal`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/IdeaBankModal.tsx) mode `inbox` — accordéons sourcing / voyage (`+N` jalons, chevron vertical) / **zoom décomposé** (jalon `x/N` + chevron horizontal, **panneau imbriqué** checklist avec barre progression).
...
| **UI Inbox** | [`InboxLineTitle.tsx`](src/components/InboxLineTitle.tsx) — 2 lignes (multiline zoom enfant), pastille catégorie ; panneau zoom jalon (progression, connecteurs, checkbox) ; **pas de miniature Vault** (perf FlatList) |
| **Hiérarchie Inbox** | [`buildInboxRootsView`](src/utils/inboxRootsView.ts) — filtrage racines **côté JS** ; accordéon sourcing + voyage + **zoom décomposé sous jalon** (`zoomInboxModel.ts`, Option B, panneau checklist) dans `IdeaBankModal` |
