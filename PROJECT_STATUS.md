...
- **Thèmes dynamiques (mai 2026)** : **TalkThemeRegistry** + showroom AsyncStorage (`@trankil_debug_theme_variant`) — 6 variantes interchangeables à la volée depuis **Debug** (🎨 EXPLORATION GRAPHIQUE) ; **Disposition Timeline** (`@trankil_debug_timeline_layout`, hub email par `category_id`) ; `useDesignTokens()` réactif ; rollback = **Actuel (TellYouTo)** / **Cartes actuelles**. Migrés : Timeline, TalkDebug, Debug, micro, `IntentionCard`, `IdeaBankModal`, `IntentionDetailSheet`. Détail : **SPEC.md § Architecture UI — 0)** et **§ 2.c–2.d**.
...
- **Corps Timeline** (sous Smart Clusters, vue Aujourd’hui + ALL + TODO) : `groupBy category_id` natif Pass 1 — blocs non vides uniquement ; lignes email avec heure + 🔁 habitude.
...
- **Planifier le début (PROJECT)** : dans [`IdeaBankModal.tsx`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/IdeaBankModal.tsx), si `type === 'PROJECT'` et pas de `project.start_date` (`getProjectStartDateFromMetadataJson`), le CTA calendrier ouvre le sélecteur **Planifier le début** (`cluster.planProjectStart`) ; à la validation : `patchMetadata` + `updateTrankilV2IntentionTemporal({ due_date })`. Autres types : `due_date` seul.
- **Actions ligne** : icônes seules ✓ / 📅 / ✏️ / 🗑️ (libellés commentés — étape 2) ; **Modifier** ferme la modale puis `openDetail` → `IntentionDetailSheet`.
