...
- **Smart Clusters (Timeline, mai–juin 2026)** : carrousel **Inbox · À acheter · Box · Routines · Projets & listes** ([`SmartClustersCarousel`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/SmartClustersCarousel.tsx)) — **Inbox** = journal 24h (sas des captures du jour) ; **Projets & listes** = hub unifié [`IdeaBankModal`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/IdeaBankModal.tsx) (`hubContext: projects`, pool `getProjectsAndLists`) ; **Box** = inventaire froid ; **Routines** = bibliothèque HABIT. Détail : **SPEC.md § 2.a bis–2.b ter**.
...
- **Pass 2 LIST/PROJECT (mai–juin 2026)** : `geminiEnrichGenericList` + routeur multi-domaine [`projectEnrich.ts`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/services/projectEnrich.ts) (`PROJECT_TRAVEL` / `PROJECT_RENOVATION` / `PROJECT_EVENT` / `PROJECT`) · brief unifié `project_brief_v2` ([`projectBriefModel.ts`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/utils/projectBriefModel.ts)). **Hub IdeaBank (juin 2026)** : Pass 2 **in-place** pour `LIST` et `PROJECT` — [`runListPass2ForHub`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/services/listPass2Hub.ts) / [`runProjectPass2ForHub`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/services/projectPass2Hub.ts) + overlay [`AIUniversalProgressOverlay`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/AIUniversalProgressOverlay.tsx) (pas de modale empilée). Capture `PROJECT` → hub direct post-Pass 1 (pas de peek intermédiaire Timeline). UI : accordéons chevron (jalons / items liste), `pass2_unlocked` après succès.
...
- `RootNavigator.tsx` : **Native Stack** racine
  - `App` → `MainStack`
  - `ProjectList` (écran legacy — **non** branché au carrousel depuis juin 2026 ; hub principal = `IdeaBankModal` `hubContext: projects`)
...
- insertion : `insertTrankilV2Intention(...)` — écrit `category_id` + `context_tag` + **`metadata_json` en un seul INSERT** (plus de `patchMetadata` post-insert) ; file sérialisée + **`PRAGMA busy_timeout=8000`** + retries sur `database is locked` / `finalizeAsync` ; log `[DATABASE] ✅ Intention sauvée avec succès | Category: … | Context: …`
...
- **Carrousel** (vue Aujourd’hui + ALL + TODO) : **Inbox · À acheter · Box · Routines · Projets & listes** — inchangé sauf tuile projets (hub unifié).
...
- **UI** : tuile carrousel + [`IdeaBankModal`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/IdeaBankModal.tsx) `hubContext: inbox` ; surface de traitement unifiée (voir § 2.d) ; sortie via Fait ✓ ou mode **Supprimer** (Phase 2 : racines + enfants sourcing/zoom).
...
#### Projets & listes — hub unifié (juin 2026)

- **Rôle** : bibliothèque **`LIST` + `PROJECT`** actifs — distinct de l’**Inbox** (sas des captures du jour uniquement).
- **SQL** : [`getProjectsAndLists`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/api/trankilV2Db.ts) — `type IN ('LIST','PROJECT')`, `is_archived = 0`, tri `created_at DESC`.
- **Carrousel** : tuile i18n **`timeline.smartClusters.projectsTitle`** (📁 Projets & listes) ; compteur = taille du pool unifié ; icône 📋 header Timeline → même hub.
- **UI** : [`IdeaBankModal`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/IdeaBankModal.tsx) `hubContext: projects` via [`hubProcessModel.ts`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/utils/hubProcessModel.ts) ; grammaire Inbox (`InboxLineTitle`, `HubTaskCheckbox`, accordéons).
- **PROJECT** : badge `+N` jalons · chevron → [`TravelMilestoneInboxRows`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/TravelMilestoneInboxRows.tsx) ; pilule « Découper en étapes » → Pass 2 in-hub + ballet overlay.
- **LIST** : badge `+N` items · chevron → lignes cochables (`list_scalable_v1`) ; pilule « Structurer la liste » → [`runListPass2ForHub`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/services/listPass2Hub.ts).
- **Capture PROJECT** : post-Pass 1 → ouverture directe hub bloc unique (`openProjectHubAccordion`) — **pas** de peek Path B sur Timeline.
- **Legacy** : [`ProjectListScreen`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/screens/ProjectListScreen.tsx) conservé dans la stack mais **non** routé depuis le carrousel.
...
### 3.1b Timeline — Smart Clusters (Inbox · Box · Routines · Projets & listes)

- [`SmartClustersCarousel.tsx`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/SmartClustersCarousel.tsx) : tuiles **Inbox · À acheter · Box · Routines · Projets & listes** ; compteur projets = `projectsListsRows.length` (`getProjectsAndLists`).
- **Inbox / hubs** : [`hubProcessModel.ts`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/utils/hubProcessModel.ts) (`hubContext`: `inbox` | `box` | `shop` | `routine` | **`projects`** | `block`) + `buildInboxRootsView` sur tout pool ; [`hubDeleteModel.ts`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/utils/hubDeleteModel.ts) ; [`IdeaBankModal`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/IdeaBankModal.tsx) — Pass 2 in-hub LIST/PROJECT ([`listPass2Hub.ts`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/services/listPass2Hub.ts), [`projectPass2Hub.ts`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/services/projectPass2Hub.ts)).
- **Box** : [`BOX_STOCK_WHERE`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/api/trankilV2Db.ts) — inventaire froid ; [`LivingHubCategoryModal`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/features/livingHub/LivingHubCategoryModal.tsx) + [`buildLivingHubBlocks`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/features/livingHub/buildLivingHubBlocks.ts) ; tap bloc → [`IdeaBankModal`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/IdeaBankModal.tsx) (purge bloc : mode Supprimer + Tout sélectionner) ; purge globale vue catégories via `LivingHubCategoryModal` (`timeline.box.*`).
...
| **Projet multi-domaine** | Brief `project_brief_v2` (voyage, rénovation, événement, move, generic) · auto Pass 2 si `auto_detail_requested` · hub + accordéon · capture PROJECT → hub direct |
| **Projet voyage** | Rétrocompat `project_brief_v1` · Pass 2 `PROJECT_TRAVEL` + packing · fallback jalons |
...
**Fichiers clés** : `src/utils/sourcingV1.ts`, `src/utils/inboxRootsView.ts`, `src/utils/projectBriefModel.ts`, `src/utils/travelProjectModel.ts`, `src/utils/jsonSalvage.ts`, `src/utils/peekOutcomeResolve.ts`, `src/services/projectEnrich.ts`, `src/services/projectPass2Hub.ts`, `src/services/listPass2Hub.ts`, `oneTapUniversalCapture.ts`, `oneTapPersist.ts`, `geminiSemanticLab.ts`, `projectMilestonesModel.ts`, `listIntentionModel.ts`, `IntentionDetailSheet.tsx`, `IntentionContext.tsx`, `offlineAudioQueue.ts`, `trankilV2Db.ts` (mapper), `IdeaBankModal.tsx`, `TimelineScreen.tsx`.
