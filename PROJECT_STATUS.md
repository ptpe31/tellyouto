...
- **TRIP — Optimisation API Maps (mai 2026)** : saisie adresse **Hybrid-Ready** (V1 manuelle : Geocoding au clic OK, autocomplete dormant via `ENABLE_AUTOCOMPLETE` dans [`features.ts`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/config/features.ts)) ; hook [`useAddressLogic`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/hooks/useAddressLogic.ts) + UI [`AddressInput`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/traffic/AddressInput.tsx) / [`AddressInputField`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/traffic/AddressInputField.tsx) ; cache Distance Matrix **grid 3 décimales (~100 m)** + persistance AsyncStorage TTL 15 min ; plafond PROBE1 **`probe1_retry_count ≥ 3` → STATIC** ; savepoints `patchMetadata` uniques en réentrance (`trankil_pm_N`). Détail : **SPEC.md § Niveau 3 Mission** et **§ Places / géoloc**.
...
- [`IdeaBankModal.tsx`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/IdeaBankModal.tsx) — Tirelire ; actions icône 40×40 + **`busyRows`** ; cinématique Éditer 320 ms ; **`onEditItem`** → `IntentionDetailSheet`.
...
| [`addressResolver.ts`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/services/addressResolver.ts) | `resolveManual` (Geocoding 1 appel/submit) ; `resolveFromPlaceId` (Place Details — futur autocomplete) |
| [`useAddressLogic.ts`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/hooks/useAddressLogic.ts) | Plomberie adresse : submit manuel, autocomplete dormant (`ENABLE_AUTOCOMPLETE`, debounce 500 ms, ≥ 4 chars, session token) |
| [`AddressInputField.tsx`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/traffic/AddressInputField.tsx) | Drop-in remplaçant `GooglePlacesAutocompleteField` (re-export rétrocompat) |
| [`features.ts`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/config/features.ts) | `ENABLE_AUTOCOMPLETE = false` (V1 manuelle) |
...
- mutations `metadata_json` : via `patchMetadata(...)` (merge sécurisé, attendu par SPEC) ; savepoints **uniques par niveau** (`trankil_pm_N`) si transaction ouverte ou appels réentrants (évite `no such savepoint` pendant PROBE1 + patch UI concurrent)
...
- **Perf** : `getItemLayout` off en mode hub.
...
- **Saisie adresse (V1)** : [`AddressInputField`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/traffic/AddressInputField.tsx) — TextInput + bouton **OK** → 1 appel Geocoding ; **0 appel** pendant la frappe (`ENABLE_AUTOCOMPLETE = false`).
