...
- **Micro « échap »** : le bouton micro reste visible (sans suggestions d’intention pendant l’overlay) ; un appui en **`pipeline_wait`** ferme seulement l’overlay (**pas** d’annulation de `submitCapturePayload` / Gemini — traitement silencieux en arrière-plan).
...
**Télémétrie badge (temporaire)** : hook [`useTripBadgeStateTelemetry`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/hooks/useTripBadgeStateTelemetry.ts) — logs `[TRIP-BADGE-STATE]` (transitions `setup → scanScheduled → elasticDeparture`, latence PROBE1, alerte lenteur > 10 s). Flag retrait : `TRIP_BADGE_STATE_TELEMETRY`.

**Robustesse Sentinel (mai 2026)** : reconcile déclenché à la sélection autocomplete (`onSelect`) et au tap Big Button « Surveiller » ; debounce 500 ms sur [`sentinelReconciler.ts`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/services/traffic/sentinelReconciler.ts), mutex activation par `intentionId`, retry SQLite [`withSentinelDbRetry`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/services/traffic/sentinelDbRetry.ts) (logs `[TRIP-SENTINEL-RECOVERY]`). `syncSentinelAfterDestinationChange` est **no-op** si `remind_to_leave === 0`. Lecture intention Sentinel via `getTrankilV2IntentionById` → `withTrankilV2Database`. Queue SQLite couvre lectures + écritures pour éviter le crash PROBE1 au GPS catch-up.
