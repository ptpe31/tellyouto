...
- Autonomie réelle en cas de panne réseau/proxy :
  ...
  - **Observabilité** : logs console **`[OFFLINE-STABILITY]`** (`logOfflineStability`) pour enqueue auto, replay, branche offline directe et **`netinfo_online_null_reachable`** ; **`[CAPTURE_FLOW]`** via [captureFlowLog.ts](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/utils/captureFlowLog.ts) : journalisation console **`__DEV__`** + émission **`notifyCapturePipelineProgress`** (`CAPTURE_PIPELINE_PROGRESS_EVENT`, `DeviceEventEmitter`) à chaque `logCaptureFlow` pour corréler `traceId` (ex. micro) avec le **dashboard de progression Talk** ; phases usuelles : `mic_stop_audio_done`, `mic_submit_invoke`, `submit_enter`, `submit_netinfo`, `peek_snapshot_emit`, `peek_snapshot_offline_queue`, `bulk_start`, `chunk_*`, `peek_first_save_emit`, **`bulk_network_resilience_enqueue`** (enqueue auto Vague 1), `submit_return_after_bulk`, `submit_offline_queued` (dont `reason: netinfo_offline`), etc.
