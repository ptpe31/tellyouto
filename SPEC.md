...
- **Accordéon zoom Inbox — Option A** (juin 2026) : après décomposition IA d’un jalon (`triggerJalonZoom`), les TASK enfants (`parent_id` = projet racine + `zoom_parent_jalon_uid`) sont groupées sous une **ancre** (`NOTE` shell `zoom_anchor_v1` ou legacy `PROJECT` enfant) via [`buildZoomInboxView`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/utils/zoomInboxModel.ts) ; clé stable `(rootProjectId, jalonUid)` — pas le `parent_id` du sous-projet ; **pas de badge `+N` à gauche** sur l’ancre — L2 interactive `Projet · décomposé · N étapes · X/Y fait` + chevron **`chevron-right`** (replié) / **`chevron-left`** (déplié) dans [`InboxLineTitle.tsx`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/InboxLineTitle.tsx) ; tap L1 → fiche ; tap L2 → liste TASK ; coexistence : projet racine voyage garde badge `+N` étapes + chevron vertical ; mode zoom ventilation : skip/coerce `PROJECT` nested (`oneTapPersist` `zoom_skip_nested_project`).
...
- **Inbox (Option A, juin 2026)** : regroupement sync via `buildZoomInboxView` ; ancre = `NOTE` shell (`zoom_anchor_v1`) créée avant bulk zoom ou legacy `PROJECT` enfant ; TASK zoom masquées des racines ; accordéon 3ᵉ mode dans `IdeaBankModal` (priorité : sourcing > zoom > voyage) ; UI ancre zoom = L2 « N étapes » + chevron horizontal (pas badge `+N` latéral).
