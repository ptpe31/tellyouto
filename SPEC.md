...
- Enrichissement Pass 2 (LIST / PROJECT) — **strictement à la demande**, **sauf exception projet voyage** :
  ...
  - Après échec : `list_enrich_status = 'error'` (+ `list_enrich_error`) ; CTA retry visible si gate encore verrouillée.
...
- Intake Share Sheet : [shareService.ts](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/services/share/shareService.ts) + [ShareIntentBootstrap.tsx](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/ShareIntentBootstrap.tsx) + Vault [fileStorage.ts](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/services/fileStorage.ts)
...
**Champ arrivée** : placeholder i18n + triangle jaune si pas d’adresse SQLite ; **pas** de préremplissage automatique depuis favoris à l’ouverture de la sheet — l’adresse n’apparaît que si `location_address` est persistée ou validée via Lazy-Fetch / Geocoding manuel. Coords invalides (`null`, `0`) rejetées par [`readValidTripCoords`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/utils/tripTripReadiness.ts).
...
- **Accordéon zoom Inbox — Option B** (juin 2026) : après décomposition IA d’un jalon (`triggerJalonZoom`), les TASK enfants (`parent_id` = projet racine + `zoom_parent_jalon_uid`) sont groupées **sous le jalon parent** dans l’accordéon voyage via [`buildZoomInboxView`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/utils/zoomInboxModel.ts) ; clé stable `(rootProjectId, jalonUid)` ; ancres `NOTE`/`PROJECT` zoom **masquées** des racines Inbox (`hiddenRootRowIds`) ; jalon décomposé : L2 `{durée} · {persona}` + pastille **`x/N`** + chevron **`chevron-right`** / **`chevron-left`** ; liste dépliée = **panneau imbriqué** (fond pastel catégorie, barre progression `x/N`, connecteurs arbre, cases à cocher carrées persistées SQLite `toggleIntentionDone`, titre 2 lignes sans NEW) ; sous-tâches DONE restent visibles via `listTrankilV2InboxZoomChildTasksForDay` ; coexistence : projet racine voyage garde badge `+N` jalons + chevron vertical ; ventilation zoom skip `PROJECT` nested (`oneTapPersist` `zoom_skip_nested_project`).
...
##### 2.d.3) Tirelire — carte TRIP « Itinéraire augmenté » (mai 2026)

**Périmètre** : intentions avec `metadata_json.trip` (y compris TASK one-tap + bloc `trip`).

**Bloc visuel** ([`IdeaBankTripItineraryBlock.tsx`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/IdeaBankTripItineraryBlock.tsx)) :
- Conteneur pastel catégorie ([`categoryPastel.ts`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/utils/categoryPastel.ts)) ;
- **Départ** : icône `map-marker-radius` + « Ma position actuelle » ou `origin_address` (ligne pressable) ;
- Connecteur pointillé vertical ;
- **Arrivée** : icône `flag-checkered` + adresse textuelle ou badge `timeline.ideaBank.tripFillAddress`.

**Rideau de recherche inline** (Search Curtain — [`IdeaBankModal.tsx`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/IdeaBankModal.tsx)) :
- Tap départ ou arrivée → overlay plein écran **sans fermer** la Tirelire (`searchTarget: { row, kind: 'origin' | 'arrival' }`) ;
- Header : retour + icône kind + titre i18n (`tripSearchOriginTitle` / `tripSearchTitle`) ;
- Champ [`AddressInputField`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/traffic/AddressInputField.tsx) (Lazy-Fetch § 8.a) ;
- Sélection → [`persistTripOriginAddress`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/services/traffic/persistTripArrivalAddress.ts) / [`persistTripArrivalAddress`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/services/traffic/persistTripArrivalAddress.ts) + `applyLocalPatch` + haptique success.

**Pilule TRIP hybride** ([`pass2IntentionCard.ts`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/utils/pass2IntentionCard.ts) + [`tripSurveillanceToggle.ts`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/services/traffic/tripSurveillanceToggle.ts)) :
...
- Bloc adresses (juste au‑dessus du bouton “Lancer l’itinéraire”) :
  - **Point de départ** :
    ...
    - Interaction : tap → [`AddressInputField`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/traffic/AddressInputField.tsx) — Lazy-Fetch (§ 8.a) : Geocoding au **Done** (< 12 car.) ou loupe (≥ 12 car.).
  ...
  - **Point d’arrivée** :
    ...
    - Interaction : tap → [`AddressInputField`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/traffic/AddressInputField.tsx) — même contrat Lazy-Fetch.
...
- **Inbox (Option B, juin 2026)** : regroupement sync via `buildZoomInboxView` ; ancres masquées des racines ; sous-tâches sous jalon voyage dans **panneau zoom** (`IdeaBankModal`) — barre progression, connecteurs, checkbox SQLite ; badge jalon `x/N` ; pas de carte ancre séparée.
...
**Share Sheet (intake image)** : en `IS_LOCAL_MODE`, l’image reste sur l’appareil (`documentDirectory/TalknDone-Vault/{intentionId}.jpg`) ; `syncVaultImageToCloudIfEnabled` / `uploadToCloudIfEnabled` sont des no-op ; l’analyse Vision passe par le même routeur `executeGeminiCall` (API Google directe). Log attendu : `[ShareService] Image reçue, envoi vers Gemini en mode LOCAL`.

### Share Sheet système → pipeline One-Tap (juin 2026)

**Objectif** : recevoir une capture d’écran (ou image) depuis le Share Sheet iOS/Android et la traiter comme une dictée One-Tap (intention persistée en SQLite), avec conservation locale de l’image dans le **TalknDone-Vault**.

**Dépendance native** : [`expo-share-intent`](https://www.npmjs.com/package/expo-share-intent) v5+ (Expo SDK 54) + [`expo-image-manipulator`](https://docs.expo.dev/versions/latest/sdk/imagemanipulator/) (compression). **Non compatible Expo Go** — dev client / build natif obligatoire (`expo prebuild` + `expo run:ios|android`).
...
| Bootstrap | [`ShareIntentBootstrap.tsx`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/ShareIntentBootstrap.tsx) | `useShareIntentContext` ; dans `CapturePresentationProvider` ; ballet overlay dès réception image ; `preassignedIntentionId` |
| Vault | [`fileStorage.ts`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/services/fileStorage.ts) | `saveAndCompressImage` (JPEG 70 %, max 1200 px) → `documentDirectory/TalknDone-Vault/{intentionId}.jpg` ; `getImagePath` ; `deleteImage` ; stub `syncVaultImageToCloudIfEnabled` |
| Vision | [`shareService.ts`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/services/share/shareService.ts) | `readLocalImageAsBase64` depuis le Vault ; `uploadToCloudIfEnabled` (stub) |
| Vision IA | [`geminiSemanticLab.ts`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/services/geminiSemanticLab.ts) | `geminiAnalyzeImageBase64` — `inlineData` + prompt : *« Analyse cette image pour extraire les intentions… »* ; opération `lab.analyze_image` |
| Orchestration | [`IntentionContext.tsx`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/context/IntentionContext.tsx) | `submitCapturePayload({ transcript, audioUri: null, preassignedIntentionId })` — **même** pipeline Bulk(1) que Phoenix / micro |
| UI consultation | [`IntentionCard.tsx`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/IntentionCard.tsx) + [`VaultImageViewerModal.tsx`](file:///Users/lala/Dev/trankil-v3/Dev-trankil-v34/src/components/VaultImageViewerModal.tsx) | Icône `image-outline` si `{intentionId}.jpg` existe ; tap → visionneuse plein écran |
...
Share Sheet OS
  ...
  → ballet overlay (onProfilerStopRecordingT0 + onPipelineDashboardOpenImmediate)
  → preassignedIntentionId = UUID
  → fileStorage.saveAndCompressImage → TalknDone-Vault/{intentionId}.jpg
  → geminiAnalyzeImageBase64 (Vision → transcript texte)
  → submitCapturePayload({ preassignedIntentionId })
  → Path A peek → runGeminiBulkSequence → insertTrankilV2Intention (talkndone.db, même id)
```

**TalknDone-Vault** :
- Dossier : `FileSystem.documentDirectory + 'TalknDone-Vault/'`.
- Nommage immuable : `{intentionId}.jpg` (lien direct intention SQLite ↔ fichier).
- Compression : JPEG qualité 70 %, largeur max 1200 px (`expo-image-manipulator`).
- Legacy : `share_intake/` conservé dans `shareService.saveSharedImageToLocal` (non utilisé par le flux Share actuel).
